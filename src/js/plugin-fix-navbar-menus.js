...
function wrapTextNodes(elm, wrapTag, tabIndex = null) {
  ...
  if (hasTextNodes && menuElm) {
    const hasChildWithTabIndex = Array.apply(null, elm.children).some(
      child => child.tabIndex > -1
    ).length;
...
export default function (hook, vm) {
  hook.doneEach(function () {
    const navbarItems = Array.apply(
      null,
      document.querySelectorAll('body > nav.app-nav > ul > li')
    );
    const sidebarItems = Array.apply(
      null,
